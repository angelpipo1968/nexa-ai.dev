...
import { EmbeddingOptimizer } from '@nexa/embeddings'
import { MultiLevelCache } from '@nexa/embeddings'
import { DomainFineTuner } from '@nexa/embeddings'
...
app.post('/api/embeddings/batch', async (c) => {
    ...
    if (parallel) {
        ...
        const results = await Promise.all(
            batches.map(batch => optimizer.optimize(batch, { modelId }))
        )
...
app.post('/api/embeddings/finetune', async (c) => {
    ...
    const tuner = new DomainFineTuner()
